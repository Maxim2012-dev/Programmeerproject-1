...
(define (maak-positie x y)
  ...
  (define (rand-horizontaal?)
    (or (= x 0) (= x (- spel-breedte 1))))
  ...
  (define (dispatch-positie msg)
    (cond
      ...
      ((eq? msg 'rand-horizontaal?) rand-horizontaal?)
      ((eq? msg 'rand-verticaal?) rand-verticaal?)
      (else (display "ongeldige boodschap - positie"))))
  dispatch-positie)
