...
(library
 ...
 (define (cdcl formula)
   ...
   (define exit '())
   (define timer (new-timer)) ;; NEW
   (define antecedents '()) ;; memorize antecedent literals (list of cons-cells)
   ...
   (define (make-true! lit interpret reason)
     ...
     (twls:time! twls lit (current-time timer)) ;; NEW: We log a timestamp
   ...
   (define (minimize-1-uip-clause confl-cls)
     ;; Mark the literals from the 1-UIP clause
     (mark-literals! interpret confl-cls)
     ;; Recursively go through antecedents of every literal
     (let* ((size (clause-size confl-cls)))
       (let ((new-clause (make-clause remaining-literals size)))
         (if (empty-clause? new-clause)
             (begin (log-info "Formula is unsatisfiable")
                    (exit #f))
             new-clause))))
                     ...
                     (let ((lit (car (literals clause))))
                       ...
                       (cond ((false? interpret lit)
                             ...
                             ((unknown? interpret lit)
                              ...
                              ;; remember literal + reason
                              (set! antecedents (cons (cons lit 'unit) antecedents)))))))
               (clauses formula))
     interpret)
     ...
     (if (has-two-or-more-lits-from-curr-dec-lvl? confl-cls)
         (let* ((l (youngest-literal confl-cls))
           ...
           (analyze-conflict (resolve confl-cls
                                      ...
                                      l
                                      not-l)))
         (minimize-1-uip-clause confl-cls)))
       ...
       (cond (conflicting-clause
             ...
             ((find-unassigned-atom interpret)
              (let ((x (find-unassigned-atom interpret)))
                ...
                ;; remember literal + reason
                (set! antecedents (cons (cons x 'dec) antecedents))))
